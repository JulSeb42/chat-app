...
export const COMMON_TEXTS = {
	...
	EMAIL_SIGNUP_BODY: (user: User, token: string) =>
		`Hello ${user.fullName},<br /><br />Thank you for creating your account on ${SITE_DATA.NAME}! <a href="${import.meta.env.ORIGIN}/verify?token=${token}&id=${user._id}">Click here to verify your account.</a>.`,

	EMAIL_RESET_PASSWORD_TITLE: `Reset your password on ${SITE_DATA.NAME}`,
	EMAIL_RESET_PASSWORD_BODY: (user: User, token: string) =>
		`Hello ${user.fullName},<br /><br />To reset your password, <a href="${import.meta.env.ORIGIN}/reset-password?token=${token}&id=${user._id}}">click here</a>.`,
